...
cam = pynokhwa.Camera(pynokhwa.query()[0])  # Create a camera
print(
    *cam.get_format_options()
    .prefer_fps_range(25, 60)
    .prefer_width_range(min_width=500)[:10],
    sep="\n",
)  # Print some of available formats
fmt = (
    cam.get_format_options()
    .prefer_fps_range(25, 60)
    .prefer_sides_ratio(800 / 504)
    .prefer_frame_format(pynokhwa.FrameFormat.MJPEG)
    .resolve()
)  # Select a format from a list of all possible ones
